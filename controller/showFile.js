...
const show = async (req, res, next) => {
  try {
    ...
    if (!file) {
      ...
      return res.status(404).json({ error: "Link have been expired" });
    }
    ...
    return res.status(200).json({
      ...
      timeLeft: `${
        Math.floor(totalMinutesLeft / 60) > 0           //if the time is negative and script.js has not deleted the file
